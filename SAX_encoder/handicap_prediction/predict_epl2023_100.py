...
def main():
    print("=" * 60)
    print("预测 2023-2024 赛季英超 - 100场比赛")
    print("=" * 60)
    ...
    print(f"\n[2] 预测结果")
    print("=" * 90)
    print(f"{'主队':<22} {'客队':<22} {'比分':<6} {'盘口':<6} {'预测':<6} {'概率':<6} {'实际':<6} {'结果'}")
    print("-" * 90)

    for _, row in df.iterrows():
        home = row['home_team'][:20] if row['home_team'] else ''
        away = row['away_team'][:20] if row['away_team'] else ''
        ...
        print(f"{home:<22} {away:<22} {score:<6} {handicap:<6} {pred:<6} {prob:<6} {actual:<6} {result}")

    print("-" * 90)

    # 统计
    pred_win = (df['pred_label'] == 1).sum()
    pred_lose = (df['pred_label'] == 0).sum()
    actual_win = (df['actual_label'] == 1).sum()
    actual_lose = (df['actual_label'] == 0).sum()

    print(f"\n【预测结果统计】")
    print(f"预测准确率: {accuracy:.2%} ({correct}/{len(df)})")
    print(f"预测赢盘: {pred_win}, 预测输盘: {pred_lose}")
    print(f"实际赢盘: {actual_win}, 实际输盘: {actual_lose}")

    # 按概率区间统计
    print(f"\n【按预测概率区间统计】")
    for thresh in [0.3, 0.4, 0.5, 0.6, 0.7]:
        high_df = df[df['pred_prob'] >= thresh]
        if len(high_df) > 0:
            high_acc = (high_df['pred_label'] == high_df['actual_label']).mean()
            print(f"  概率>={thresh}: {len(high_df)}场, 准确率{high_acc:.2%}")
