...
def analyze_single_bookmaker(matches, bookmaker_name, output_config):
    ...
    # 4. 参数建议逻辑 (优化后的推荐)
    # 基于覆盖率测试结果，推荐使用interleaved策略以获得更好的覆盖率
    suggested_strategy = "interleaved"  # 优化覆盖率推荐

    # interleaved策略推荐参数
    suggested_word_size = 8  # w=8-10
    if p50 > 30:
        suggested_word_size = 10
    elif p50 < 15:
        suggested_word_size = 6

    suggested_alphabet_size = 4  # a=4 (比8更集中)
    ...
    print("最终系统配置建议")
    print("=" * 70)
    print(f"  - strategy (编码策略): {suggested_strategy} (优化覆盖率)")
...
def analyze_odds_data(json_file, output_config="sax_config.json", bookmaker=None):
    ...
    if os.path.isdir(json_file):
        # 目录模式: 分析所有庄家
        base_output_dir = (
            os.path.dirname(output_config)
            if output_config != "sax_config.json"
            else json_file
        )
        ...
        if "bookmakers" in sample_match and isinstance(
            sample_match["bookmakers"], list
        ):
            # 使用第一个有数据的庄家
            for bm in sample_match["bookmakers"]:
                if bm.get("runningOdds") and len(bm["runningOdds"]) >= 2:
                    return analyze_single_bookmaker(
                        matches, bm["bookmakerName"], output_config
                    )
