...
2. 复用 Playwright 批量下载 bet365 欧赔数据
3. 保存到 rawdata/odds/odds 目录

用法：
    python download_all_odds.py [--limit N] [--match-id ID] [--dry-run]
"""
...
def download_all_odds_batch(
    downloader: DataDownloader,
    match_ids: list[int],
    output_dir: str = "./rawdata/odds/odds",
    limit: int | None = None,
    dry_run: bool = False,
    delay: float = 0.3,
):
    """
    批量下载 Bet365 欧赔数据（复用浏览器）
    ...
    try:
        ...
        for i, match_id in enumerate(match_ids, 1):
            ...
            try:
                ...
                # 使用复用浏览器下载
                # 欧赔 URL: https://1x2.titan007.com/OddsHistory.aspx?sid={match_id}&cid=281
                url = f"https://1x2.titan007.com/OddsHistory.aspx?sid={match_id}&cid={company_id}"
...
def main():
    parser = argparse.ArgumentParser(
        description="批量下载 Bet365 欧赔数据",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
    # 下载所有比赛的 Bet365 欧赔数据
    python download_all_odds.py

    # 只下载前 10 个
    python download_all_odds.py --limit 10

    # 只下载指定比赛
    python download_all_odds.py --match-id 2799893

    # 试运行（不实际下载）
    python download_all_odds.py --dry-run
        """,
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="./rawdata/odds/odds",
        help="输出目录 (默认: ./rawdata/odds/odds)",
    )
    parser.add_argument(
    ...
    # 下载欧赔数据（复用浏览器）
    result = download_all_odds_batch(
